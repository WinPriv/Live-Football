...
public class AppLovinStarRatingView extends View {
    ...
    private void drawFilledStars(Canvas canvas, Path path) {
        ...
        canvas.drawRect(new RectF(gl.Code, gl.Code, (roundUpToNearestHalfInt * 5.0f * this.sideLength) + ceil, getHeight()), filledStarPaint);
    }

    private Path getCombinedPathForAllStarsWithSide(float f) {
        PointF pointF = new PointF(gl.Code, this.paddingDp);
        float f10 = pointF.x + this.paddingDp;
        Path path = new Path();
        for (int i10 = 0; i10 < 5; i10++) {
            float f11 = pointF.y;
            Path starPath = getStarPath(new RectF(f10, f11, f10 + f, f11 + f));
            f10 += this.interStarsSpacingDp + f;
    ...
    private List<PointF> getStarPointsOnACircle(PointF pointF, float f, float f10) {
    ...
    private void setupPaints() {
        ...
        paint.setColor(g.a(i10, getContext()));
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(DEFAULT_STROKE_WIDTH);
        Paint paint2 = filledStarPaint;
        paint2.setColor(g.a(i10, getContext()));
