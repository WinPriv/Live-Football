package com.facebook.ads.redexgen.X;

import androidx.annotation.Nullable;

/* renamed from: com.facebook.ads.redexgen.X.Fi, reason: case insensitive filesystem */
/* loaded from: assets/audience_network.dex */
public final class C0885Fi {
    public final float A00;
    public final float A01;
    public final float A02;
    public final float A03;
    public final int A04;
    public final int A05;
    public final int A06;

    @Nullable
    public final String A07;

    public C0885Fi(@Nullable String str) {
        this(str, Float.MIN_VALUE, Float.MIN_VALUE, Integer.MIN_VALUE, Integer.MIN_VALUE, Float.MIN_VALUE, Integer.MIN_VALUE, Float.MIN_VALUE);
    }

    public C0885Fi(@Nullable String str, float f, float f10, int i10, int i11, float f11, int i12, float f12) {
        this.A07 = str;
        this.A01 = f;
        this.A00 = f10;
        this.A05 = i10;
        this.A04 = i11;
        this.A03 = f11;
        this.A06 = i12;
        this.A02 = f12;
    }
}
